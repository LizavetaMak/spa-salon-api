...
async function authenticate({username, password}) {
    let sql = `select user.id_user, user.email, user.name, user.phone, user.birthday_user\n` +
        `from user,sign_in\n` +
        `where login="${username}"  and pasword="${password}" and sign_in.id_user= user.id_user;`;
   ...
   const result= await poolPromise.execute(sql);
            ...
            if (user.length!==0) {
                const token = jwt.sign({sub: user.id_user, role: user.role}, config.secret);
                return {
                    user,
                    token
                };
